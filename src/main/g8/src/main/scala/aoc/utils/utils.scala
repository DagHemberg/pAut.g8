...
extension [A](a: A)
  ...
  /** Performs a flood-fill using a neighbour function `f: A => Set[A]`. Returns a set of all elements generated by the function with the starting point `a`. */
  def expand(f: A => Set[A]): Set[A] = Set(a).converge(s => s ++ s flatMap f)
...
extension (tup: Pos2D)
  ...
  infix def dot(other: Vec2): Double = tup.toVector dot other.toVector
  def magnitude: Double = tup.toVector.magnitude
